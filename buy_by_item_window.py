"""
Окно «Покупки по товару»: выбор товара или группы товаров (категории),
показ количества закупок, суммы закупок и графика по датам.
"""
...
class BuyingsByItemWindow:
    def __init__(self, parent):
        ...
        self.window.title("Покупки по товару")
    ...
    def create_widgets(self):
        ...
        self.label_count = ttk.Label(stats_frame, text="Количество закупок: —", style='Result.TLabel')
        self.label_count.pack(anchor=tk.W)
        self.label_revenue = ttk.Label(stats_frame, text="Сумма закупок: —", style='Result.TLabel')
    ...
    def analyze(self):
        ...
        try:
            if self.mode_var.get() == "category":
                daily, total_count, total_revenue = self.analyzer.get_buyings_by_categories_daily(
                    selection, start_db, end_db)
            else:
                daily, total_count, total_revenue = self.analyzer.get_buyings_by_items_daily(
                    selection, start_db, end_db)

            self.label_count.config(text=f"Количество закупок: {total_count}")
            self.label_revenue.config(text=f"Сумма закупок: {total_revenue:,.0f} руб.")
    ...
    def draw_graph(self, daily):
        ...
        ax1.set_ylabel('Сумма закупок (руб.)', fontsize=10)
        ax1.set_xlabel('Дата', fontsize=10)
        ax1.set_title('Сумма закупок по датам', fontsize=12, fontweight='bold')
        ax1.set_xticks(x)
        ...
        ax2 = ax1.twinx()
        ax2.plot(x, counts, color='coral', marker='o', markersize=4, linewidth=1.5, label='Кол-во закупок')
        ax2.set_ylabel('Количество закупок', fontsize=10, color='coral')
