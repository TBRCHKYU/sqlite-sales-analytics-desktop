"""
Окно анализа закупок по аккаунтам (аналог sales_accounts_window для закупок).
"""
...
class BuyingsAnalysisWindow:
    def __init__(self, parent):
        ...
        self.window.title("Анализ закупок по аккаунтам")
    ...
    def create_timeline_tab(self):
        """Создает вкладку с временной шкалой закупок"""
    ...
    def load_date_range(self):
        """Загрузка диапазона дат из базы данных (закупки)"""
        try:
    ...
    def analyze_accounts(self):
        """Анализ закупок по аккаунтам за выбранный период"""
        ...
        try:
            ...
            account_buyings = self.analyzer.get_account_buyings_summary(start_date_db, end_date_db)
            ...
            self.display_summary(account_buyings, account_metrics, start_date_db, end_date_db)
    ...
    def display_summary(self, account_buyings, account_metrics, start_date, end_date):
        """Отображение сводной статистики по закупкам"""
        for item in self.summary_tree.get_children():
            self.summary_tree.delete(item)
        
        total_buyings = sum(acc['total'] for acc in account_buyings.values())
        account_count = len(account_buyings)
        transaction_count = sum(acc['count'] for acc in account_buyings.values())
        ...
        avg_daily = total_buyings / days_in_period if days_in_period > 0 else 0
        avg_per_account = total_buyings / account_count if account_count > 0 else 0
        
        sorted_accounts = sorted(account_buyings.items(), 
                               key=lambda x: x[1]['total'], 
                               reverse=True)
        top_3_accounts = sorted_accounts[:3]
        
        self.summary_tree.insert("", "end", values=("Общая статистика", "", ""))
        self.summary_tree.insert("", "end", values=("  Общая сумма закупок", f"{total_buyings:,.0f} руб.", ""))
        ...
        for i, (account, data) in enumerate(top_3_accounts, 1):
            percent = (data['total'] / total_buyings * 100) if total_buyings > 0 else 0
    ...
    def display_timeline(self, monthly_timeline):
        """Отображение временной шкалы закупок"""
        ...
        totals = [monthly_timeline[m]['total'] for m in months]
        ...
        bars1 = ax.bar(x - width/2, totals, width, label='Сумма закупок', color='skyblue', edgecolor='black')
        
        ax2 = ax.twinx()
        bars2 = ax2.bar(x + width/2, account_counts, width, label='Количество аккаунтов', color='lightcoral', edgecolor='black', alpha=0.7)
        
        ax.set_xlabel('Месяц')
        ax.set_ylabel('Сумма закупок (руб.)', color='blue')
    ...
    def display_comparison(self, comparison_data):
        """Отображение сравнения аккаунтов по закупкам"""
        ...
        ax1.set_title('Распределение закупок по топ-аккаунтам', fontsize=10, fontweight='bold')
        ...
        ax2.set_ylabel('Сумма закупок (руб.)')
        ax2.set_title('Сравнение сумм закупок', fontsize=10, fontweight='bold')
        ...
        if comparison_data['monthly_data'] and comparison_data['all_months']:
            ...
            ax3.set_ylabel('Сумма закупок (руб.)')
            ax3.set_title('Динамика закупок по месяцам (топ-5 аккаунтов)', fontsize=10, fontweight='bold')
            ax3.legend()
